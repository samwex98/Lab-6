...
public class PokerTableController implements Initializable {
	...
	private PathTransition CreatePathTransition(Point2D fromPoint, Point2D toPoint, ImageView img) {
		...
		path.getElements().add(new CubicCurveTo(0, 120, 0, 240, 380, 180));
		path.getElements().add(new CubicCurveTo(toPoint.getX() + 50, toPoint.getY() - 100, toPoint.getX() - 69,
				toPoint.getY() + 69, toPoint.getX() + 420, toPoint.getY() - 120));
		path.getElements().add(new MoveTo(fromPoint.getX() - 21, fromPoint.getY() + 100));
		path.getElements().add(new CubicCurveTo(1, 0, 2, 0, 3, 4));
